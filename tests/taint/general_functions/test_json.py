...
class TestJsonLoads:
    ...
    def test_loads_empty_and_edge_cases(self):
        ...
        # Empty object
        tainted_empty = TaintWrapper("{}", taint_origin=["test"])
        result = json.loads(tainted_empty)
        assert isinstance(result, TaintWrapper)
        assert get_taint_origins(result) == ["test"]

        # Null values
        tainted_null = TaintWrapper('{"value": null}', taint_origin=["test"])
        result = json.loads(tainted_null)
        assert isinstance(result, TaintWrapper)
        assert result["value"] is None

        # Special characters
        tainted_special = TaintWrapper('{"text": "Hello\\nWorld\\t!"}', taint_origin=["test"])
    ...
    def test_loads_with_tainted_json(self):
        ...
        # Create TaintWrapper with taint
        json_str = TaintWrapper('{"secret": "password123"}', taint_origin=["user"])
...
class TestJsonDumps:
    ...
    def test_dumps_basic_taint_propagation(self):
        ...
        obj = {"name": TaintWrapper("Alice", taint_origin=["user_input"]), "age": 30}

        result = json.dumps(obj)

        # Result should be a TaintWrapper
        assert isinstance(result, TaintWrapper)
        assert get_taint_origins(result) == ["user_input"]
        ...
        obj = TaintWrapper(
            {
                "user": TaintWrapper(
                    {
                        "name": TaintWrapper("Bob", taint_origin=["source1"]),
                        "email": TaintWrapper("bob@test.com", taint_origin=["source2"]),
                    },
                    taint_origin=["source1"],
                ),
                "metadata": TaintWrapper(
                    [
                        TaintWrapper("tag1", taint_origin=["source3"]),
                        TaintWrapper("tag2", taint_origin=["source3"]),
                    ],
    ...
    def test_dumps_mixed_tainted_untainted(self):
        ...
        mixed_obj = {
            "tainted": TaintWrapper("secret", taint_origin=["api"]),
            "clean": "public",
            "number": 42,
        }
    ...
    def test_dumps_with_tainted_data(self):
        ...
        # Create object with TaintWrapper that has taint
        obj = {"data": TaintWrapper("sensitive123", taint_origin=["user"])}
...
class TestJsonRoundTrip:
    ...
    def test_roundtrip_preserves_taint(self):
        ...
        original_json = TaintWrapper('{"message": "hello world"}', taint_origin=["original"])
    ...
    def test_roundtrip_with_multiple_taint_sources(self):
        ...
        obj = {
            "field1": TaintWrapper("value1", taint_origin=["source1"]),
            "field2": TaintWrapper("value2", taint_origin=["source2"]),
        }
...
class TestJsonEdgeCases:
    ...
    def test_loads_invalid_json(self):
        ...
        tainted_invalid = TaintWrapper('{"invalid": json}', taint_origin=["test"])
    ...
    def test_loads_with_custom_parameters(self):
        ...
        tainted_json = TaintWrapper('{"name": "test"}', taint_origin=["hook_test"])
    ...
    def test_dumps_with_custom_parameters(self):
        ...
        obj = {"message": TaintWrapper("hello", taint_origin=["test"])}
...
class TestJsonIntegrationWithOtherPatches:
    ...
    def test_json_with_re_patch_output(self):
        ...
        tainted_from_re = TaintWrapper("extracted_data", taint_origin=["regex_result"])
    ...
    def test_json_boolean_handling(self):
        ...
        tainted_json = TaintWrapper('{"enabled": true, "disabled": false}', taint_origin=["bool_test"])
        result = json.loads(tainted_json)
        ...
        # Should not be tainted since no TaintWrapper values
...
class TestJsonEdgeCasesExtended:
    ...
    def test_empty_containers(self):
        ...
        # Empty object
        tainted = TaintWrapper("{}", taint_origin=["empty"])
        ...
        # Empty array
        tainted = TaintWrapper("[]", taint_origin=["empty_array"])
        result = json.loads(tainted)
        assert isinstance(result, TaintWrapper)
        assert len(result) == 0
        ...
        # Empty string value
        tainted = TaintWrapper('{"empty": ""}', taint_origin=["empty_str"])
        result = json.loads(tainted)
    ...
    def test_large_objects_and_arrays(self):
        ...
        # Add taint and test
        large_data["metadata"] = TaintWrapper("large_test", taint_origin=["performance"])
        json_str = json.dumps(large_data)
        assert isinstance(json_str, TaintWrapper)
    ...
    def test_taint_with_whitespace(self):
        ...
        compact = TaintWrapper('{"key":"value"}', taint_origin=["compact"])
    ...
    def test_circular_reference_handling(self):
        ...
        circular_dict = TaintWrapper({"name": "test"}, taint_origin=["circular"])
    ...
    def test_custom_json_encoder_decoder(self):
        ...
        obj = {"data": TaintWrapper("test", taint_origin=["custom"]), "tags": {1, 2, 3}}
    ...
    def test_extreme_nesting(self):
        ...
        nested = TaintWrapper("deep_value", taint_origin=["deep"])
        for i in range(10):
    ...
    def test_malformed_json_error_preservation(self):
        ...
        malformed_cases = [
            TaintWrapper('{"missing_quote: "value"}', taint_origin=["bad1"]),
            TaintWrapper('{"trailing_comma": "value",}', taint_origin=["bad2"]),
            TaintWrapper(
                '{"unescaped": "line\nbreak"}', taint_origin=["bad3"]
            ),  # Invalid unescaped newline
            TaintWrapper("[1,2,3,]", taint_origin=["bad4"]),  # Trailing comma in array
        ]
    ...
    def test_dumps_with_taint_complex(self):
        ...
        # Create object with multiple TaintWrapper values having different taint
        secret1 = TaintWrapper("password123", taint_origin=["user1"])
        secret2 = TaintWrapper("token_abc", taint_origin=["user2"])
    ...
    def test_json_with_different_separators(self):
        ...
        obj = {"key1": TaintWrapper("value1", taint_origin=["sep_test"]), "key2": "value2"}
    ...
    def test_json_indentation_taint(self):
        ...
        obj = {"level1": {"level2": TaintWrapper("nested_value", taint_origin=["indent_test"])}}
    ...
    def test_json_sort_keys_with_taint(self):
        ...
        obj = {
            "zebra": TaintWrapper("last", taint_origin=["sort1"]),
            "alpha": TaintWrapper("first", taint_origin=["sort2"]),
            "beta": "middle",
        }
    ...
    def test_json_with_none_values(self):
        ...
        obj = {
            "data": TaintWrapper("important", taint_origin=["important"]),
            "optional": None,
            "nested": {"value": None, "other": TaintWrapper("other", taint_origin=["other"])},
        }

        json_str = json.dumps(obj)

        # Should be tainted due to TaintWrapper values
        assert isinstance(json_str, TaintWrapper)
    ...
    def test_json_ensure_ascii_parameter(self):
        ...
        obj = {"unicode": TaintWrapper("café", taint_origin=["unicode_test"])}
    ...
    def test_multiple_taint_sources_complex(self):
        ...
        obj = TaintWrapper(
            {
                "user_data": TaintWrapper(
                    {
                        "username": TaintWrapper("alice", taint_origin=["user_input"]),
                        "email": TaintWrapper("alice@test.com", taint_origin=["email_validation"]),
                    },
                    taint_origin=["user_session"],
                ),
                "system_data": {
                    "timestamp": TaintWrapper("2023-01-01", taint_origin=["system_time"]),
    ...
    def test_json_with_skipkeys_parameter(self):
        ...
        obj = {
            "valid_key": TaintWrapper("valid_value", taint_origin=["valid"]),
