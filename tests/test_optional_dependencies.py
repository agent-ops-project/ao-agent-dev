...
class TestOptionalDependencies:
    def test_graceful_import_handling(self):
        ...
        try:
            ...
            # Import your module (this should work)
            ...
            for patch_func in CUSTOM_PATCH_FUNCTIONS:
                try:
                    ...
                    pytest.fail(f"Function {patch_func.__name__} should handle missing dependencies gracefully")
                except Exception as e:
                    # Other exceptions might be OK depending on your code
    ...
    def test_import_with_api_dependencies(self):
        ...
        try:
            # Import your module normally (no mocking)
            from runtime_tracing.monkey_patches import CUSTOM_PATCH_FUNCTIONS
            ...
            for patch_func in CUSTOM_PATCH_FUNCTIONS:
                try:
                    ...
                    print(f"✗ {patch_func.__name__} failed with ImportError: {e}")
                    pytest.fail(f"Function {patch_func.__name__} failed with ImportError when dependencies should be available: {e}")
                except Exception as e:
                    # Other exceptions might be expected behavior
                    print(f"? {patch_func.__name__} failed with other error: {e}")
                    # Depending on your requirements, you might want to fail here too:
                    # pytest.fail(f"Function {patch_func.__name__} failed unexpectedly: {e}")
