...
def test_import_without_api_dependencies():
    ...
    modules_to_remove = [
        mod for mod in sys.modules if any(api in mod for api in ["openai", "anthropic", "google"])
    ]
    original_modules = {}
    ...
    try:
        ...
        print(
            f"✓ Successfully imported monkey_patches with {len(CUSTOM_PATCH_FUNCTIONS)} patch functions"
        )
