...
export const MiniGraphView: React.FC<GraphViewProps> = ({ nodes: initialNodes, edges: initialEdges, onNodeUpdate }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [nodes, setNodes, onNodesChange] = useNodesState([]);
  const [edges, setEdges, onEdgesChange] = useEdgesState([]);
  ...
  const updateLayout = useCallback(() => {
    ...
    const maxY =
      Math.max(...Array.from(positions.values()).map((pos) => pos.y)) + 60;
    setContainerHeight(maxY);
    const routedEdges = routeEdges(initialEdges, positions);
    const flowNodes: Node[] = initialNodes.map((node) => {
      return {
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', height: containerHeight, minHeight: 120 }}
    >
      <ReactFlowProvider>
