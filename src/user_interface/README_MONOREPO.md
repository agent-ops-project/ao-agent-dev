...
│   ├── package.json
│   └── webpack.config.js
└── webapp/                # Web application workspace
    ├── client/            # Vite + React frontend
    │   ├── src/
    │   └── package.json   # Now references vscode-extension as workspace dependency
...
From the `src/user_interface/` directory, run:
...
3. **Rebuild extension**: `cd vscode-extension && npm run compile`

## Old Structure

The previous structure has been backed up to `src/user_interface_old/` (if you need to reference it).
