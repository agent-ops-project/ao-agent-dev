...
def bird_pre_process_function(batch: dict, args):
    ...
    inputs = [
        bird_get_input(question=question, serialized_schema=serialized_schema, prefix=prefix)
...
class Constructor(object):
    ...
    def to_seq2seq(self, raw_datasets: DatasetDict, cache_root: str):
        ...
        if getattr(self.args.seq2seq, "few_shot_rate"):
            raw_train = random.sample(
                list(raw_datasets["train"]),
                int(self.args.seq2seq.few_shot_rate * len(raw_datasets["train"])),
            )
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            for raw_data in tqdm(self.raw_datasets):
                ...
                extend_data.update(
                    {
                        "struct_in": extend_data["serialized_schema"].strip(),
                        "text_in": question,
                        "seq_out": seq_out,
                    }
                )
