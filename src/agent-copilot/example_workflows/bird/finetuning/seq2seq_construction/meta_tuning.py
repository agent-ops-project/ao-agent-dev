...
class MultiTaskWrapper(Dataset):
    def __init__(self, args_path2dataset, meta_args, section):
        if meta_args.load_multiple_prefix_module_weights_from:
            task_id2task_name = sorted(
                [
                    "_".join(task_name.split("_")[:-1])
                    for task_name, module_weight_location in meta_args.load_multiple_prefix_module_weights_from
                ]
            )
            meta_args.task_id2task_name = task_id2task_name
            meta_args.task_name2task_id = {
                task_name: task_id for task_id, task_name in enumerate(task_id2task_name)
            }
        ...
        if temp and temp != 1 and section == "train":
            ...
            for args_path in sorted(args_path2data.keys()):
                args_path2data[args_path] = upsample(
                    args_path2data[args_path], args_path2upsample[args_path]
                )
        ...
        for args_path, data in args_path2data.items():
            for item in data:
                ...
                if meta_args.load_multiple_prefix_module_weights_from:
                    item["task_id"] = meta_args.task_name2task_id[
                        os.path.basename(args_path)[: -len(".cfg")]
                    ]
        ...
        if section == "dev" and meta_args.dataset.eval_num:
            for args_path in args_path2data.keys():
                ...
                if eval_num < len(full_data):
                    ...
                    args_path2data[args_path] = [
                        full_data[int(idx * stride)] for idx in range(eval_num)
                    ]
    ...
    def to_seq2seq(self, raw_datasets_dict: Dict[str, DatasetDict]):
        ...
        for arg_path, dataset in raw_datasets_dict.items():
            if len(dataset) == 2:
                ...
                train_dev_test_data_of_tasks["test"][arg_path] = dataset[
                    1
                ]  # Use the dev set if no test set.
            elif len(dataset) == 3:
                train_dev_test_data_of_tasks["train"][arg_path] = dataset[0]
                train_dev_test_data_of_tasks["validation"][arg_path] = dataset[1]
...
class TrainDataset(Dataset):
    ...
    def __init__(self, meta_args, tasks_train_data: Dict[str, Dataset]):
        ...
        self.meta_training_data = MultiTaskWrapper(
            args_path2dataset=tasks_train_data, meta_args=meta_args, section="train"
        )
...
class DevDataset(Dataset):
    ...
    def __init__(self, meta_args, tasks_dev_data):
        ...
        self.meta_dev_data = MultiTaskWrapper(
            args_path2dataset=tasks_dev_data, meta_args=meta_args, section="dev"
        )
...
class TestDataset(Dataset):
    ...
    def __init__(self, meta_args, tasks_test_data):
        ...
        self.meta_test_data = MultiTaskWrapper(
            args_path2dataset=tasks_test_data, meta_args=meta_args, section="test"
        )
