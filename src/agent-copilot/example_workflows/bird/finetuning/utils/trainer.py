...
class EvaluateFriendlySeq2SeqTrainer(transformers.trainer_seq2seq.Seq2SeqTrainer):
    ...
    def prediction_step(
        ...
        gen_kwargs = {
            "max_length": (
                self._max_length if self._max_length is not None else self.model.config.max_length
            ),
            "num_beams": (
                self._num_beams if self._num_beams is not None else self.model.config.num_beams
            ),
    ...
    def _post_process_function(
        ...
        return EvalPrediction(
            predictions=predictions, items=[examples[idx] for idx in range(len(predictions))]
        )
