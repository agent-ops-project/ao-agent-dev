...
class RowDeleteTruncate(TableTruncate):
    ...
    def estimate_delete_ratio(self, table_content: Dict, question: str):
        ...
        question_tokens = self.tokenizer.tokenize(question)  # special tokens are counted outside.
        # calculate the tokens of header
        header_string = self.table_linearize.process_header(table_content["header"])
        header_tokens = self.tokenizer.tokenize(header_string)
    ...
    def delete_unrealted_rows(
        ...
        drop_items = min(
            len(truncated_unrelated_indices), int(len(table_content["rows"]) * delete_ratio)
        )
        ...
        if "id" in table_content and len(drop_row_indices) > 0:
            logger.warning(
                "Delete {:.2f} rows in table {}".format(len(drop_row_indices), table_content["id"])
            )
