    ...
    def __getitem__(self, index):
        ...
        if raw_item["text_in"]:
            ...
            if self.conv_sep in raw_item["text_in"]:
                ...
                struct_in_norm = re.sub(
                    "  +", " ", self.graph_pedia[raw_item["graph_idx"]]["new_struct_in"]
                )
        ...
        tokenized_inferred_input_ids[
            tokenized_inferred_input_ids == self.tokenizer.pad_token_id
        ] = -100

        item = {
            "input_ids": torch.LongTensor(tokenized_question_and_schemas.data["input_ids"]),
            "attention_mask": torch.LongTensor(
                tokenized_question_and_schemas.data["attention_mask"]
            ),
        ...
        if self.args.model.use_description and self.args.model.map_description:
            ...
            item["description_input_ids"] = torch.LongTensor(
                tokenized_description.data["input_ids"]
            )
            item["description_attention_mask"] = torch.LongTensor(
                tokenized_description.data["attention_mask"]
            )
        ...
        if self.args.model.knowledge_usage == "separate":
            ...
            item["knowledge_attention_mask"] = torch.LongTensor(
                tokenized_knowledge.data["attention_mask"]
            )

        # assertion:
        if (
            len([a for a in tokenized_question_and_schemas.input_ids if a > 1])
            != self.graph_pedia[int(item["graph_idx"])]["graph"].number_of_nodes()
        ):
            print("index: {}".format(index))
            print(
                "len of tokens is {}".format(
                    len([a for a in tokenized_question_and_schemas.input_ids if a > 1])
                )
            )
            print(
                "len of nodes is {}".format(
                    self.graph_pedia[int(item["graph_idx"])]["graph"].number_of_nodes()
                )
            )
