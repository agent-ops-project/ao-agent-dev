...
def main() -> None:
    ...
    if not args.arg_paths:
        ...
        seq2seq_dataset_split: tuple = utils.tool.get_constructor(args.seq2seq.constructor)(
            args
        ).to_seq2seq(raw_datasets_split, cache_root)
    else:
        cache_root = os.path.join("output", "cache")
        os.makedirs(cache_root, exist_ok=True)
        meta_tuning_data = {}
        for task, arg_path in args.arg_paths:
            ...
            task_seq2seq_dataset_split: tuple = utils.tool.get_constructor(
                task_args.seq2seq.constructor
            )(task_args).to_seq2seq(task_raw_datasets_split, cache_root)
    ...
    train_dataset = (
        TokenizedDataset(args, training_args, model_tokenizer, seq2seq_train_dataset, graph_pedia)
        if seq2seq_train_dataset
        else None
    )
    eval_dataset = (
        TokenizedDataset(args, training_args, model_tokenizer, seq2seq_eval_dataset, graph_pedia)
        if seq2seq_eval_dataset
        else None
    )
    test_dataset = (
        TokenizedDataset(args, training_args, model_tokenizer, seq2seq_test_dataset, graph_pedia)
    ...
    if training_args.load_weights_from:
        state_dict = torch.load(
            os.path.join(training_args.load_weights_from, transformers.WEIGHTS_NAME),
            map_location="cpu",
        )
    ...
    if args.load_multiple_prefix_module_weights_from:
        ...
        for task_name, module_weight_location in args.load_multiple_prefix_module_weights_from:
            state_dict = torch.load(
                os.path.join(module_weight_location, transformers.WEIGHTS_NAME), map_location="cpu"
            )
            ...
            for weight_name, stored_tensor in state_dict.items():
                ...
                reconstruct_state_dict[
                    "{}.{}.{}".format(
                        MULTI_PREFIX_ATTR_NAME, "_".join(task_name.split("_")[:-1]), weight_name
                    )
                ] = stored_tensor
