...
def aggregate_prompt(past_prompt_dict: OrderedDict, task_names_list=None, strategy="simple_concat"):
    """
    ...
    if strategy in ["simple_separate", "separate_with_new_prefix"]:
        ...
        for task_name in task_names_list:
            ...
            if not constructed_prompt:
                constructed_prompt = [
                    {k: {_k: _v for _k, _v in v.items()} for k, v in item.items()}
                    for item in prompt_of_this_task
                ]
                continue
            for layer_number, prompt_of_this_task_in_this_layer in enumerate(prompt_of_this_task):
                ...
                for prompt_pos in [
                    ...
                    for key_value_attention_mask in [
                        ...
                        if key_value_attention_mask == "prev_key_padding_mask":
                            constructed_prompt_layer[prompt_pos][key_value_attention_mask] = (
                                torch.cat(
                                    [
        ...
        for layer_number, _ in enumerate(new_prefix):
            ...
            for prompt_pos in [
                ...
                for key_value_attention_mask in [
                    ...
                    if key_value_attention_mask == "prev_key_padding_mask":
                        constructed_prompt_layer[prompt_pos][key_value_attention_mask] = torch.cat(
                            [
                                new_prefix[layer_number][prompt_pos][key_value_attention_mask],
                                constructed_prompt_layer[prompt_pos][key_value_attention_mask],
                            ],
                            dim=1,
                        )
...
class Model(PushToHubFriendlyModel):
        ...
        self.multi_prefix = nn.ModuleDict(
            {
                task_name: nn.ModuleDict(
                    {
                        "wte": nn.Embedding(prefix_len, self.n_embd),
                        "control_trans": nn.Sequential(
                            nn.Linear(self.n_embd, self.mid_dim),
                            nn.Tanh(),
                            nn.Linear(self.mid_dim, self.match_n_layer * 2 * self.n_embd),
                        ),
                        "wte_enc": nn.Embedding(prefix_len, self.n_embd),
                        "control_trans_enc": nn.Sequential(
                            nn.Linear(self.n_embd, self.mid_dim),
                            nn.Tanh(),
                            nn.Linear(self.mid_dim, self.match_n_layer * 2 * self.n_embd),
                        ),
                        "wte_dec": nn.Embedding(prefix_len, self.n_embd),
    ...
    def get_prompt(
        ...
        old_bsz = bsz
        bsz = bsz * sample_size
        input_tokens = torch.arange(prefix_len).long().unsqueeze(0).expand(bsz, -1)
        input_tokens = (
            input_tokens.to("cuda") if torch.cuda.is_available() else input_tokens.to("cpu")
        )
        ...
        past_key_values_dec = self.multi_prefix[task_name]["dropout"](past_key_values_dec)
        past_key_values_dec = past_key_values_dec.permute([2, 0, 3, 1, 4]).split(2)

        # Encoder prefix
        input_tokens_enc = torch.arange(prefix_len).long().unsqueeze(0).expand(old_bsz, -1)
        input_tokens_enc = (
            input_tokens_enc.to("cuda") if torch.cuda.is_available() else input_tokens_enc.to("cpu")
        )
        ...
        **kwargs,
        ...
        for (
            ...
            all_past_prompt[task_name] = self.get_prompt(
                bsz=(
                    1
                    if self.args.model.prefix_agg_strategy
                    in ["simple_separate", "separate_with_new_prefix"]
                    else bsz
                ),
                task_name=task_name,
        ...
        task_names_list = [
            self.args.task_id2task_name[task_id.item()] for task_id in kwargs.pop("task_ids")
        ]
    ...
    def generate(self, input_ids, attention_mask, **kwargs):
        ...
        # do the agg of this prompt(key, value and padding mask)
        past_prompt = aggregate_prompt(
            all_past_prompt, task_names_list, strategy=self.args.model.prefix_agg_strategy
        )
