...
class RGAT_Tuning(nn.Module):
    ...
    def forward(self, hidden_states, graph_batch, relation_embedding):
        ...
        for i, graph in enumerate(graph_batch):
            hidden_states[i] = self.graph_caption_one(
                hidden_states[i], graph["graph"], graph["edges"], relation_embedding
            )
    ...
    def graph_caption_one1(self, hidden_state, graph_node_idx, graph, edges, relation_emb):
        ...
        """
        graph_node_idx_select = torch.tensor(
            [idx for idx in graph_node_idx if idx >= 0], device=graph_node_idx.device
        )
...
class RGAT_Layer(nn.Module):
    ...
    def __init__(self, ndim, edim, num_heads=1, feat_drop=0.2):
        ...
        self.affine_q, self.affine_k, self.affine_v = (
            nn.Linear(self.ndim, dim),
            nn.Linear(self.ndim, dim, bias=False),
            nn.Linear(self.ndim, dim, bias=False),
        )
    ...
    def forward(self, x, lgx, graph):
        ...
        q, k, v = (
            self.affine_q(self.feat_dropout(x)),
            self.affine_k(self.feat_dropout(x)),
            self.affine_v(self.feat_dropout(x)),
        )
        e = (
            lgx.view(-1, self.num_heads, self.d_k)
            if lgx.size(-1) == q.size(-1)
            else lgx.unsqueeze(1).expand(-1, self.num_heads, -1)
        )
        with g.local_scope():
            g.ndata["q"], g.ndata["k"] = q.view(-1, self.num_heads, self.d_k), k.view(
