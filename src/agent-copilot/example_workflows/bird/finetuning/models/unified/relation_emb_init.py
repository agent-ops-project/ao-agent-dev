...
class Relation_init:
    ...
    def relation_init_prompt(
        ...
        for k, v in self.rel2id.items():
            tokenized_relation_prompt = self.tokenizer(
                self.relation_prompt_mapping[k],
                padding="max_length",
                truncation=True,
                max_length=1024,
            )
            relation_prompt_input_ids = torch.LongTensor([tokenized_relation_prompt.input_ids]).to(
                device
            )
            tmp_relations_input_ids.append(relation_prompt_input_ids)
            relation_prompt_attention_mask = torch.LongTensor(
                [tokenized_relation_prompt.attention_mask]
            ).to(device)
