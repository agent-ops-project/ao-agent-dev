...
class Model(PushToHubFriendlyModel):
    def __init__(self, args):
        ...
        if self.args.model.knowledge_usage == "separate":
            self.knowledge_trans = nn.Sequential(
                ...
                nn.Tanh(),
                nn.Linear(
                    self.mid_dim, self.match_n_layer * 2 * self.match_n_head * self.match_n_embd
                ),
            )
    ...
    def get_prompt(
        self, bsz=None, sample_size=1, graph_representation=None, description=None, knowledge=None
    ):
        old_bsz = bsz
        bsz = bsz * sample_size
        ...
        if knowledge is not None:
            past_key_values = torch.cat(
                [
                    past_key_values,
                    self.knowledge_trans(knowledge.repeat_interleave(sample_size, dim=0)),
                ],
                dim=1,
            )
        ...
        if knowledge is not None:
            past_key_values_dec = torch.cat(
                [
                    past_key_values_dec,
                    self.knowledge_trans_dec(knowledge.repeat_interleave(sample_size, dim=0)),
                ],
                dim=1,
            )
    ...
    def get_graphix_encoder(self, input_ids, kwargs):
        node_emb_batch = self.pretrain_model.encoder(
            input_ids
        ).last_hidden_state  # [bsz x input_max_length x d_model]
        graph_batch = self.graph_factory(kwargs)
        hidden_states = self.rgat_layer(
            node_emb_batch, graph_batch, self.relation_embedding
        )  # [bsz x input_max_length x d_model]
        pdb.set_trace()
...
class T5LayerRGAT(nn.Module):
    ...
    def graph_caption(self, hidden_states, graph_batch, relation_emb):
        ...
        for i, graph in enumerate(graph_batch):
            hidden_states[i] = self.graph_caption_one(
                hidden_states[i], graph["graph"], graph["edges"], relation_emb
            )
