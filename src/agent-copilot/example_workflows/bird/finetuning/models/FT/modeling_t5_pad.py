        ...
        if position_bias is None:
            ...
            if prefix is None:
                ...
                position_bias = torch.cat(
                    [
                        torch.zeros(
                            (1, self.n_heads, int_seq_length, key_states.shape[2] - key_length)
                        ).to(position_bias.device),
                        position_bias,
                    ],
                    dim=3,
                )  # TODO: Chen
            ...
            if mask is not None:
                if prefix is None:
                    ...
                    mask = torch.cat(
                        [
                            torch.zeros(
                                (batch_size, 1, mask.shape[2], key_states.shape[2] - key_length)
                            ).to(mask.device),
                            mask,
                        ],
                        dim=3,
                ...
                position_bias = (
                    position_bias + mask
                )  # (batch_size, n_heads, seq_length, key_length + prefix_length)  # TODO: Chen
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            cross_attn_prefix = (
                past_prompt[i]["cross_attention_prompt"] if past_prompt else None
            )  # TODO: Chen
