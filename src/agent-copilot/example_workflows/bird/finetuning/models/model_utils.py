...
def rnn_wrapper(encoder, inputs, lens, cell="lstm"):
    ...
    out = sorted_out.new_zeros(*out_shape).scatter_(
        0, sort_key.unsqueeze(-1).unsqueeze(-1).repeat(1, *out_shape[1:]), sorted_out
    )
    h_shape = list(sorted_h.size())
    h_shape[1] = total_num
    h = sorted_h.new_zeros(*h_shape).scatter_(
        1, sort_key.unsqueeze(0).unsqueeze(-1).repeat(h_shape[0], 1, h_shape[-1]), sorted_h
    )
    if cell.upper() == "LSTM":
        c = sorted_c.new_zeros(*h_shape).scatter_(
            1, sort_key.unsqueeze(0).unsqueeze(-1).repeat(h_shape[0], 1, h_shape[-1]), sorted_c
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        ), "Head num %d must be divided by hidden size %d" % (num_heads, hidden_size)
    ...
    def forward(self, hiddens, query_hiddens, mask=None):
        ...
        Q, K, V = (
            self.W_q(self.feat_drop(query_hiddens)),
            self.W_k(self.feat_drop(hiddens)),
            self.W_v(self.feat_drop(hiddens)),
        )
        Q, K, V = (
            Q.reshape(-1, Q.size(1), 1, self.num_heads, self.d_k),
            K.reshape(-1, 1, K.size(1), self.num_heads, self.d_k),
            V.reshape(-1, 1, V.size(1), self.num_heads, self.d_k),
        )
        ...
        concat = (
            (a.unsqueeze(-1) * V).sum(dim=2).reshape(-1, query_hiddens.size(1), self.hidden_size)
        )
...
class PoolingFunction(nn.Module):
    ...
    def __init__(self, hidden_size=256, output_size=256, bias=True, method="attentive-pooling"):
        ...
        self.mapping_function = (
            nn.Sequential(nn.Linear(hidden_size, output_size, bias=bias), nn.Tanh())
