...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if prompt is not None and attention_mask is not None:
            ...
            bsz = (
                bsz_input
                if bsz_input == bsz_prefix
                else AssertionError("In Encoder, the bsz of input and prefix must be the same")
            )
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        if prompt is not None and encoder_attention_mask is not None:
            ...
            bsz_prefix, _, prefix_seq_length, _ = prompt[0]["cross_attention_prompt"][
                "prev_key"
            ].size()
            bsz = bsz_input if bsz_input == bsz_prefix else None
            # if bsz is None:
            ...
            encoder_attention_mask = torch.cat(
                [
                    torch.zeros(bsz, 1, tgt_seq_len, prefix_seq_length).to(
