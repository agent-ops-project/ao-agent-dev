...
class EvaluateTool(object):
    ...
    def run_sqls_parallel(self, sqls, db_places, num_cpus=1):
        ...
        for i, sql in enumerate(sqls):
            ...
            print(sql)
            pool.apply_async(
                self.execute_model, args=(sql, db_places[i], i), callback=self.result_callback
            ).get()
        pool.close()
        pool.join()
    ...
    def evaluate(self, preds, golds, section):
        ...
        exec_accuracy = self.compute_execution_accuracy(
            gt_results=gold_results, predict_results=pred_results
        )
