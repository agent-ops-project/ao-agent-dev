...
class LLMCallTransformer(cst.CSTTransformer):
    ...
    def __init__(self, llm_patterns: Dict[str, List[str]], source_file_path: str = ""):
        ...
        self.sanitized_vars = set()  # Track variables that contain sanitized values (None, empty tuples, etc.)
        self.sanitized_calls_in_current_assign = False  # Track if current assignment RHS was sanitized
        
        # Line number mapping: transformed_line -> original_line
        self.line_mapping: Dict[int, int] = {}
        self.transformation_locations: List[Tuple[int, str, str]] = []  # (line, original_pattern, transformed_pattern)
        
        # Compile pattern regexes for efficiency
        self.compiled_patterns = {}
        for provider, patterns in llm_patterns.items():
            self.compiled_patterns[provider] = [
                re.compile(pattern.replace('.', r'\.').replace('*', '.*'))
    ...
    def _record_transformation(self, node: cst.CSTNode, original_pattern: str, transformed_pattern: str):
    ...
    def visit_FunctionDef(self, node: cst.FunctionDef) -> None:
        ...
        self.current_function = f"{self.current_class}.{node.name.value}" if self.current_class else node.name.value
        return True
    ...
    def leave_Assign(self, original_node: cst.Assign, updated_node: cst.Assign) -> cst.Assign:
        ...
        if self.current_class and len(updated_node.targets) == 1:
            ...
            if isinstance(target.target, cst.Attribute):
                ...
                if (isinstance(attr.value, cst.Name) and 
                    ...
                    if (self._is_llm_related_assignment(updated_node.value) or
                        any(pattern in attr_name.lower() for pattern in ["openai", "anthropic", "client", "llm", "ai"])):
                        
                        # Transform to module-level variable with better naming
                        if attr_name.lower() in ["openai", "anthropic"] and not attr_name.endswith("_client"):
        ...
        if len(updated_node.targets) == 1 and isinstance(updated_node.targets[0].target, cst.Subscript):
            target = updated_node.targets[0]
            sub = target.target
            # Only handle simple string key subscripts: y["key"]
            if (
                isinstance(sub.slice, cst.Index) and
                isinstance(sub.slice.value, cst.SimpleString)
            ):
                ...
                if base_name:
                    ...
                    self._record_transformation(original_node, f"{base_name}['{key_str}']", new_var_name)
    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.Call:
        ...
        if call_chain and call_chain.endswith(".parse_standard_response") and any(prefix in call_chain for prefix in ["LANGUAGE_MODEL", "LanguageModel"]):
            self.transformation_count += 1
            ...
            return cst.Call(
                func=cst.Name("extract_llm_response"),
        ...
        if isinstance(updated_node.func, cst.Attribute):
            if self._is_llm_method_call(updated_node):
                ...
                self._record_transformation(original_node, f"method.{method_name}", "call_llm(generic)")
                # Track that this function contains LLM calls
                if self.current_function:
                    self.methods_with_llm_calls.add(self.current_function)
        ...
        if isinstance(updated_node.func, cst.Name):
            if updated_node.func.value == "llm":
                ...
                content_arg = updated_node.args[0].value if updated_node.args else cst.SimpleString('"default"')
                return cst.Call(
                    func=cst.Name("call_llm"),
                    args=[
                        cst.Arg(value=cst.SimpleString('"generic"')),
                        cst.Arg(value=content_arg)
                    ]
                )
        
        # Custom wrapper: LanguageModel.invoke -> treat as LLM call
        if call_chain and call_chain.endswith(".invoke") and any(prefix in call_chain for prefix in ["LANGUAGE_MODEL", "LanguageModel"]):
        ...
        if call_chain and call_chain.endswith(".load_tomli_str") and any(prefix in call_chain for prefix in ["LANGUAGE_MODEL", "LanguageModel"]):
            self.transformation_count += 1
    ...
    def leave_Subscript(self, original_node: cst.Subscript, updated_node: cst.Subscript) -> Union[cst.Subscript, cst.Name]:
        """
    ...
    def leave_Attribute(self, original_node: cst.Attribute, updated_node: cst.Attribute) -> Union[cst.Attribute, cst.Name]:
        ...
        if isinstance(updated_node.value, cst.Name):
            ...
            if var_name in self.sanitized_vars:
                ...
                self.transformation_count += 1
                self._record_transformation(original_node, f"{var_name}.{updated_node.attr.value}", "None")
                return cst.Name("None")
        ...
        if (isinstance(updated_node.value, cst.Name) and 
            ...
            if any(attr_name.lower().startswith(pattern) for pattern in llm_client_attrs):
                ...
                client_name = f"{attr_name}_client" if not attr_name.endswith("client") else attr_name
                self.member_variables[attr_name] = True
    ...
    def _is_llm_related_assignment(self, value_node: cst.BaseExpression) -> bool:
        ...
        if isinstance(value_node, cst.Call):
            ...
            if call_chain:
                ...
                llm_keywords = [
                    "client", "openai", "anthropic", "llm", "ai", 
                    "gpt", "claude", "chat", "completion"
                ]
    ...
    def _is_llm_method_call(self, call_node: cst.Call) -> bool:
        ...
        if isinstance(call_node.func, cst.Attribute):
            ...
            llm_method_patterns = [
                "process_request", "make_call", "create", "query", "call", 
                "process", "run", "invoke", "request"
            ]
    ...
    def _arg_contains_llm_content(self, arg: cst.Arg) -> bool:
        ...
        # Check for LLMRequest objects or similar
            ...
            if isinstance(arg.value.func, cst.Name):
                if "llm" in arg.value.func.value.lower() or "request" in arg.value.func.value.lower():
                    return True
    ...
    def _is_sanitized_value(self, value_node: cst.BaseExpression) -> bool:
        ...
        # Tuple of None values (from parse_standard_response sanitization)
            ...
            return all(isinstance(element, cst.Element) and isinstance(element.value, cst.Name) and element.value.value == "None" for element in value_node.elements)
...
'''
            ...
            if self.transformation_count > 0:
                ...
                if self.sanitized_vars:
                    print(f"🧼 Sanitized {len(self.sanitized_vars)} variables to eliminate false positives")
            
            # Inject stub for extract_llm_response if needed (TITO helper)
            if "extract_llm_response(" in transformed_code and "def extract_llm_response(" not in transformed_code:
                extract_def = '''
...
def transform_code_for_pyre(source_code: str, llm_patterns: Optional[Dict[str, List[str]]] = None) -> str:
    """
...
def analyze_transformations(source_code: str) -> Dict[str, Any]:
    ...
    patterns = {
        "openai": ["openai.chat.completions.create", "client.chat.completions.create", "openai.responses.create", "client.responses.create"],
...
'''
    ...
    for t in analysis['transformations']:
        print(f"- {t['type']}: {t.get('original', '')} -> {t.get('transformed', t.get('details', ''))}") 