...
def process_files(files_to_process: List[Path], target: Path, temp_dir: Path, config: Dict[str, Any]) -> Dict[str, List]:
...
def _process_files_parallel(files: List[Path], target: Path, temp_dir: Path, 
    ...
    def process_file(source_file):
        try:
            ...
            return source_file.name, transformed_code != source_code, str(source_file), transformations
...
def _process_files_sequential(files: List[Path], target: Path, temp_dir: Path, 
                            config: Dict[str, Any]) -> Dict[str, List]:
    ...
    for source_file in files:
        try:
            ...
            if was_transformed and transformer.transformation_count > 0:
                ...
                print(f"📊 Tracked {len(transformer.call_llm_vars)} variables with call_llm results")
            else:
                print(f"   📄 {source_file.name}: {status}")
