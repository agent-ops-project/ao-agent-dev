...
def setup_pyre_environment(temp_dir: Path, config: Dict[str, Any], line_mappings: Optional[Dict[str, List]] = None) -> None:
...
def run_pyre_analysis(work_dir: Path, config: Dict[str, Any], use_daemon: bool = True, 
                     changed_files: Optional[List[str]] = None) -> Tuple[List[Dict[str, Any]], float]:
...
def _run_incremental_analysis(pyre_binary: str, config: Dict[str, Any], 
                            changed_files: Optional[List[str]] = None, 
                            original_cwd: Optional[Path] = None) -> List[Dict[str, Any]]:
        ...
        if taint_output_file.exists():
            ...
            try:
                ...
                if isinstance(results, list):
                    ...
                    print(f"🎯 Found {len(llm_flows)} LLM-related flows (filtered from {len(results)} total)")
                    return llm_flows
                else:
...
def _run_standard_analysis(pyre_binary: str, config: Dict[str, Any], 
                           original_cwd: Optional[Path] = None) -> List[Dict[str, Any]]:
...
def analyze_codebase_incremental(target_path: str, config: Dict[str, Any], 
    ...
    try:
        ...
        results, duration = run_pyre_analysis(
            temp_dir, 
            config, 
            use_daemon=True,  # Use daemon for incremental benefits
            changed_files=changed_files  # Pass changed files for filtering
        )
...
def _is_llm_flow(result: Dict[str, Any]) -> bool:
    ...
    llm_keywords = [
        "llm", "llminput", "llmoutput", "call_llm",
        "openai", "anthropic", "completions", "chat.completions"
    ]
