...
def _create_enhanced_results(results: List[Dict[str, Any]], target_path: str, timestamp: str) -> Dict[str, Any]:
    ...
    for i, result in enumerate(results, 1):
        enhanced_flow = {
            ...
            "description": result.get('description', result.get('message', 'LLM data flow detected')),
    ...
    return {
        ...
        "summary": {
            ...
            "unique_sources": len(set(r.get('source_details', {}).get('location', 'unknown') 
                                    for r in results if 'source_details' in r)),
            "unique_sinks": len(set(r.get('sink_details', {}).get('location', 'unknown') 
                                  for r in results if 'sink_details' in r))
        },
