...
from pyre_runner import setup_pyre_environment, run_pyre_analysis, cleanup_pyre_server, analyze_codebase_incremental


def analyze_codebase(target_path: str, config: Dict[str, Any], use_daemon: bool = True) -> List[Dict[str, Any]]:
...
def analyze_codebase_incremental_wrapper(target_path: str, config: Dict[str, Any], 
                                       changed_files: List[str] = None) -> List[Dict[str, Any]]:
...
def _map_results_to_original_line_numbers(results: List[Dict[str, Any]], 
                                        line_mappings: Dict[str, List], 
                                        target: Path) -> None:
            ...
            for source_path, transformations in line_mappings.items():
                ...
                if target.is_dir():
                    try:
                        ...
                        if str(relative_path) == path_str or source_file.name == Path(path_str).name:
