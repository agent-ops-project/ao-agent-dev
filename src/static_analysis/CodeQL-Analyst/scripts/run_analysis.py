...
class CodeQLAnalyzer:
    ...
    def check_installation(self):
        ...
        result = subprocess.run([str(self.codeql_path), "version"], 
    ...
    def create_database(self, source_path, db_name=None):
        ...
        cmd = [
            str(self.codeql_path), "database", "create",
            str(db_path),
            "--language=python",
    ...
    def run_query(self, database_path, query_file=None):
        ...
        results_file = self.results_dir / f"results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sarif"
        
        print(f"🔍 Running query: {query_file}")
        
        cmd = [
            str(self.codeql_path), "database", "analyze",
            str(database_path),
            str(query_file),
                        ...
                        for thread_flow in flow.get('threadFlows', []):
                            ...
                            for step in thread_flow.get('locations', []):
                                ...
                                path.append({
                                    ...
                                    'message': step.get('location', {}).get('message', {}).get('text', '')
                                })
