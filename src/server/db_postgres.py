    ...
    if _shared_conn is None:
        with _db_lock:
            ...
            if _shared_conn is None:
                ...
                if not database_url:
                    raise ValueError(
                        "DATABASE_URL environment variable is required for Postgres connection"
                    )
...
def _init_db(conn):
    ...
    # Migration: Add user_id column to experiments if it doesn't exist
    try:
        c.execute(
            """
            DO $$ 
            BEGIN
                IF NOT EXISTS (
                    ...
                    ALTER TABLE experiments ADD COLUMN user_id TEXT;
                END IF;
            END $$;
            """
        )
        ...
        logger.warning(f"Migration warning (user_id column): {e}")
        conn.rollback()
