...
if USE_POSTGRES:
    ...
    from aco.server.db_postgres import (
        get_conn,
        query_one,
        query_all,
        execute,
    ...
    # Use SQLite (original implementation below)
    ...
    def _init_db(conn):
        ...
        conn.commit()
        # Ensure `user_id` column exists for backward compatibility (if table created before)
        try:
            c.execute("PRAGMA table_info(experiments)")
            cols = [r[1] for r in c.fetchall()]
            if "user_id" not in cols:
                try:
                    c.execute("ALTER TABLE experiments ADD COLUMN user_id INTEGER")
                    conn.commit()
                    logger.info("Added user_id column to experiments table")
                except Exception as e:
                    logger.warning(f"Failed to add user_id column: {e}")
        except Exception:
            # If PRAGMA fails for any reason, ignore and continue
            pass
