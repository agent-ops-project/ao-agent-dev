...
The wrappers containt the origin of their "taint" (i.e., which LLM call they stem from) and implement functions such as `__add__`, so when you have `str + TaintStr` or `TaintStr + TaintStr`, taint is propagated through these operators. 
