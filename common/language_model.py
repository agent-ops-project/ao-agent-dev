...
from common.utils import abs_path
...
class LanguageModel:
    def __init__(self):
        ...
        self.config = tomllib.load(open(abs_path("../configs/main.toml"), "rb"))
        ...
        if self.llm.is_openai() or self.low_complexity_llm.is_openai():
            ...
            key_file_path = abs_path("../configs/keys/openai.key")
    ...
    # def log_traj(self, prompt, system_msg, response, cache_key, file_name=None):
    #     if file_name is None:
    #         file_name = "/Users/ferdi/Documents/llm-code-assist/fundraising_crm_traj.jsonl"
    ...
    def invoke(
        ...
        if cache_key is not None:
            ...
            if cached is not None:
                ...
                self.log_traj(cache_key=cache_key, system_msg=system_msg, prompt=prompt, response=cached)
                return cached
        if self.verbose:
        ...
        # Log trajectory.
        self.log_traj(cache_key=cache_key, system_msg=system_msg, prompt=prompt, response=response)
        return response
