    ...
    for folder in all_folders:
        if os.path.isdir(f'./data/{folder}') and not folder.startswith('__') and not folder.startswith('.') and not folder.startswith('data'):
            ...
            if len(system_answers) != len(jsonlines): 
                logger.warning(f"Mismatch in number of answers and questions in folder {folder}. Skipping.")
                continue
    return 
...
def print_eval_scores(eval_system, result_dir="."):
    ...
    type_acc = {key: 1 - val['wr'] / val['total'] if val['total'] > 0 else 0.0 for key, val in type_counts.items()}
    file_acc = {key: val['cor'] / val['total'] if val['total'] > 0 else 0.0 for key, val in file_counts.items()}
    print("\n--- Evaluation Results ---")
