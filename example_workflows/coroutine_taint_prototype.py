import asyncio
from aco.runner.taint_wrappers import TaintWrapper, taint_wrap

async def process_data(data: str) -> str:
    """User function that should propagate taint through string operations"""
    assert isinstance(data, TaintWrapper), "data is not tainted"
    return await asyncio.sleep(0.5)

async def test_taint_preservation():
    tainted_input = taint_wrap("secret", taint_origin=["user_input"])
